...
# query
```{r, include=FALSE}
DB_string = "Driver={SQL Server};Server=NLLARSQL53;Database=FLEXLIM_COVID_V6_BELGIUM_ANALYSE;Trusted_Connection=Yes"
...
first_part <- function(q_num){
  ...
  first_part[12] = ",CASE"
	first_part[13] = "  WHEN q3_1.Suspect_COVID_Vaccin_Vac_Date2_Date IS NOT NULL THEN CONVERT(DATE, q3_1.Suspect_COVID_Vaccin_Vac_Date2_Date)"
	first_part[14] = "  WHEN q4_1.Suspect_COVID_Vaccin_Vac_Date2_Date IS NOT NULL THEN CONVERT(DATE, q4_1.Suspect_COVID_Vaccin_Vac_Date2_Date)"
	first_part[15] = "  WHEN q5_1.Suspect_COVID_Vaccin_Vac_Date2_Date IS NOT NULL THEN CONVERT(DATE, q5_1.Suspect_COVID_Vaccin_Vac_Date2_Date)"
	first_part[16] = "END AS 'vaccine_date2'"
  return(first_part)
}
...
ae_matrix = matrix(0, nrow = 6, ncol = 10) # enddate index matrix, equal to 1 if present, 2 if present and enddate is new, 0 else. rows represent the questionnaire and columns represent the ae variables. 
ae_matrix2 = matrix(0, nrow = 6, ncol = 10) # startdate index matrix, similar to ae_matrix.
ae_matrix_qol = matrix(0, nrow = 6, ncol = 10) # qol index matrix, similar to ae_matrix.
for(q_num in 1:6){ # check what enddate, startdate and qol variables there are based on variables in questionnaire 1 - 6
    query = paste("SELECT *",
                  paste("FROM [Vragenlijst ", q_num, " onderzoek coronavaccin_1]", sep = ""),
                  sep = "") # setup query that returns all variables from questionnaire q_num
...
# 
# test = split_names[[262]]
# 
# 
# (any(var_names %in% test) + "enddate" %in% test) * 1 - "value" %in% test
```
...
write.table(test[[2]],
            file = "S:\\Gebruikers\\Thomas\\projects\\covid query\\main covid queries\\INT_sd_query.txt",
            quote = F,
            row.names = F,
            col.names = F) # save query as .txt file
...
second_part_other_llt <- function(q_num, q_num_start, new, type, enddate_index, qol_index){
  type3 = type2 = type
  # change type if extensiveswelling is used
  if(type == "extensiveswelling_injectionsiter"){
    type = type3 = "injectionsiter"
...
full_query_other_llts <- function(q_num, q_num_start, new, type, enddate_ind, qol_ind, vaccine_index1){
  #add all query parts together
  first = first_part(q_num = q_num_start, vaccine_index1)
  second = second_part_other_llt(q_num = q_num, 
                                 q_num_start = q_num_start, 
                                 new = new,
                                 type = type,
...
get_data_other_ltts <- function(){
  ...
  for(j in 1:length(types)){
    for(k in 1:6){
      ...
      if(ae_matrix_startdate[k,j] != 0){
        queries = c(queries, full_query_other_llts(k, k, k, types[j], (ae_matrix_end + ae_matrix_new_end)[,j], ae_matrix_qol[,j], vaccine_index1))
