...
#query
```{r}
databases = c("BELGIUM", "FRANCE", "ITALY", "UK")

get_all_data <- function(database){
  DB_string = paste("Driver={SQL Server};Server=NLLARSQL53;Database=FLEXLIM_COVID_V6_", database, "_ANALYSE;Trusted_Connection=Yes", sep = "")
...
for(i in 1:length(databases)){
  df_list[[i]] = get_all_data(databases[i])
}
```
...
dose_df <- function(df){
  ...
  start_adr_date <- str_extract(df$startdate, '[0-9]{1,2}-[0-9]{1,2}-[0-9]{4}')
  #start_adr_date
  start_adr_date <- as.Date(start_adr_date, format="%m-%d-%Y")
  #start_adr_date
  tto_days <- str_extract(df$startdate, '[0-9]{1,}.Days')
  tto_days <- as.numeric(gsub("([0-9]+).*$", "\\1", tto_days))
  tto_hours <- str_extract(df$startdate, '[0-9]{1,}.Hours')
  tto_hours <- as.numeric(gsub("([0-9]+).*$", "\\1", tto_hours))
  tto_weeks <- str_extract(df$startdate, '[0-9]{1,}.Weeks')
  ...
  startdate_tto[is.na(startdate_tto)] = survey_complete_date[is.na(startdate_tto)]
...
for(i in 1:length(databases)){
  print(i)
  df_list[[i]] = dose_df(df_list[[i]])
  print(sum(is.na(df_list[[i]]$dose)))
}
...
#double side effects
```{r}
double_side_effects_list = list()

double_side_effects_func <- function(df){
  ...
  return(double_sideeffects)
}

for(i in 1:length(databases)){
  double_side_effects_list[[i]] = double_side_effects_func(df_list[[i]])
}
...
# save 
```{r}
out_list = list('BELGIUM' = double_side_effects_list[[1]],
                'FRANCE' = double_side_effects_list[[2]],
                'ITALY' = double_side_effects_list[[3]],
                'UK' = double_side_effects_list[[4]])

fname <- paste(
  "S:\\Kernproces Monitoring\\1. LIM inhoudelijk\\3. Onderzoeken\\1. Lopend\\Corona LIM\\Data extractie & analyse\\1.extractie\\",
  "double_side_effects_INT.xlsx"
  ,sep=""
)
